...
1. **all** as _default rule_: builds the project, compiles all `.c` files into `.o`, then **creates** the program (`minishell`)
2. **clean** : removes compiled object files (`.o`)
3. **fclean** : _clean_ rule and removes the executable (`minishell`)
...
_Note : the `minishell` project works on its own and uses the [libft](https://github.com/bibickette/libft) library. Since the library is added as a submodule, you need to clone the repository with it._
...
_Project validation date : October 2, 2024_
