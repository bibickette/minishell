		...
		t_command *content)
{
	...
	while (*actual)
	{
		if ((*actual_content)->type == OUT_REDIR_TYPE
			|| (*actual_content)->type == HD_APPEND_TYPE)
		{
			content->redirections = add_redirection(content->redirections,
					(*actual_content)->str, &redir_size);
			increment_actual(actual, actual_content);
			if (*actual && (*actual_content)->type == OUTFILE_TYPE)
				content->outputs = add_file(content->outputs,
						(*actual_content)->str, &output_size);
		}
		else if ((*actual_content)->type == IN_REDIR_TYPE
			|| (*actual_content)->type == HERE_DOC_TYPE)
		{
			content->redirections = add_redirection(content->redirections,
					(*actual_content)->str, &redir_size);
			increment_actual(actual, actual_content);
			if (*actual && (*actual_content)->type == INFILE_TYPE)
				content->inputs = add_file(content->inputs,
						(*actual_content)->str, &input_size);
		}
...
t_list	*command_listing(t_list *token)
{
	...
	while (actual)
	{
		...
		while (1)
		{
			if (!content)
				return (NULL);
			else if (actual_content->type == CMD_TYPE
				|| actual_content->type == BUILTIN_TYPE)
				process_command(&actual_content, content);
			else if (actual && (actual_content->type == ARG_TYPE
					|| actual_content->type == OPT_TYPE))
				process_arguments_and_options(&actual, &actual_content,
					content);
			else if (actual && (actual_content->type == IN_REDIR_TYPE
					|| actual_content->type == OUT_REDIR_TYPE
					|| actual_content->type == HERE_DOC_TYPE
					|| actual_content->type == HD_APPEND_TYPE
					|| actual_content->type == PIPE_TYPE))
