...
/*   Updated: 2024/09/20 20:16:26 by phwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	do_the_dup(t_data *minishell, t_list *token, t_cmd *cmd)
{
	...
	else
	{
		if (dup2(minishell->pipe_fd[cmd->index - 1][READ], STDIN_FILENO) < 0
			|| dup2(minishell->pipe_fd[cmd->index][WRITE], STDOUT_FILENO) < 0)
		{
			perror(DUP_ERR);
...
void	dup_last_cmd(t_data *minishell, t_list *token, t_cmd *cmd)
{
	...
	path = NULL;
	if (open_all_outfile(minishell, cmd) == KO || \
	redirection_out(cmd, cmd->files, STDOUT_FILENO) != OK)
		exit(execve_error_free(minishell, path, token));
	if (dup2(minishell->pipe_fd[cmd->index - 1][READ], STDIN_FILENO) < 0)
	{
		perror(DUP_ERR);
